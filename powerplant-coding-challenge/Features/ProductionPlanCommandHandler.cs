...
{
    public class ProductionPlanCommandHandler(ProductionManager productionManager, ProductionPlanValidator productionPlanValidator) : IRequestHandler<ProductionPlanCommand, List<ProductionPlanCommandResponse>>
    {
        private readonly ProductionManager _productionManager = productionManager;
        private readonly ProductionPlanValidator _productionPlanValidator = productionPlanValidator;
        ...
        public async Task<List<ProductionPlanCommandResponse>> Handle(ProductionPlanCommand command, CancellationToken cancellationToken)
        {
            ...
            _productionPlanValidator.ValidateTotalCapacity(command.Powerplants, command.Load);
            _productionPlanValidator.ValidateLoadAgainstPmin(command.Powerplants, command.Load);
            ...
            _productionManager.EnsureTotalProductionMatchesLoad(response, command.Load, command.Powerplants);
